...
run: prereqs  ## Runs the site like it runs in production
	$(NPM) run webpack
	$(NPM) run ts-compile
	env NODE_ENV=production $(NODE) $(NODE_ARGS)  -r ts-node/register --experimental-vm-modules --experimental-loader=ts-node/esm --loader ts-node/esm ./out/dist/app.js --webpackContent ./out/webpack/static $(EXTRA_ARGS)

.PHONY: dev
dev: prereqs ## Runs the site as a developer; including live reload support and installation of git hooks
	./node_modules/.bin/supervisor -w app.js,lib,etc/config,static/tsconfig.json -e 'js|ts|node|properties|yaml' -n exit --exec $(NODE) $(NODE_ARGS) --  -r ts-node/register --experimental-vm-modules --experimental-loader=ts-node/esm --loader ts-node/esm ./app.js $(EXTRA_ARGS)
...
debug: prereqs ## Runs the site as a developer with full debugging; including live reload support and installation of git hooks
	./node_modules/.bin/supervisor -w app.js,lib,etc/config,static/tsconfig.json -e 'js|ts|node|properties|yaml' -n exit --inspect 9229 --exec $(NODE) $(NODE_ARGS) --  -r ts-node/register --experimental-vm-modules --experimental-loader=ts-node/esm --loader ts-node/esm./app.js --debug $(EXTRA_ARGS)
